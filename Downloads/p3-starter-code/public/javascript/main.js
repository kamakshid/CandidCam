...
(function() {
  ...
  function connect_to_chat_firebase(){
    ...
    fb_instance = new Firebase("https://blazing-fire-5309.firebaseio.com/");
    ...
    fb_instance_stream.on("child_added",function(snapshot){
      ...
      var messageParts = (snapshot.val().m).split(":");
...
//        },1000);
         ...
         if(counter < 3) {
            counter = 3;
            $("#num_requests").text(counter);
            $("#notification_center").html('<p id="replenished">Whoohoo, your reaction requests have been replenished!</p>');
            setTimeout(function(){ $("#replenished").fadeOut(slow); }, 3000);
         }
         setTimeout(function(){fb_instance_stream.push({m:username+": ", v:cur_video_blob, c: my_color})}, 3000);
      }
    });
...
//        if(($(this).val()).search(re) != -1){
        if($('#request').is(':checked')) {
            if(counter == 0) {
               $("#notification_center").html("<p id='run_out'>You are out of reaction requests! You must wait until your chat partner requests your reaction before you can request again.</p>");
                setTimeout(function(){ $("#run_out").fadeOut(slow); }, 3000);
            }else {
               counter--;
               $("#num_requests").text(counter);
            ...
            fb_instance_stream.push({m:username+": " +$(this).val(), c: my_color, r: false});
        }
        $(this).val("");
        scroll_to_bottom(0);
      }
